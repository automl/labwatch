...
from labwatch.converters.convert_to_configspace import sacred_space_to_configspace, \
    sacred_config_to_configspace, configspace_config_to_sacred

def simple_sp():
    batch_size = UniformNumber(lower=32, upper=64, default=32, type=int)
    num_units_first_conv = UniformNumber(lower=32, upper=64, default=32, type = int)
    num_units_second_conv = UniformNumber(lower=32, upper=64, default=32, type=int)
...
def space_with_condition():
    ...
    units_second = UniformNumber(lower=32,
                                 upper=64, default=32, type = int) | Condition(n_layers, [two])
    dropout_second = UniformNumber(lower=0.2, upper=0.8,
                                   default=0.5, type = float) | Condition(n_layers, [2])
